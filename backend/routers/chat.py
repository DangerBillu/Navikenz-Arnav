...
def create_chat(payload: CreateChatRequest, db: Session = Depends(get_db), user=Depends(get_current_user)):
    return chat_services.create_chat(db, user.id, payload.title)


@router.get("", response_model=list[ChatResponse])
def get_user_chats(db: Session = Depends(get_db), user=Depends(get_current_user)):
    return chat_services.get_user_chats(db, user.id)
...
def get_user_chat_dashboard(db: Session = Depends(get_db), user=Depends(get_current_user)):
    return chat_services.get_dashboard_stats(db, user.id)


@router.get("/{chat_id}", response_model=ChatDetailResponse)
def get_chat(chat_id: str, db: Session = Depends(get_db), user=Depends(get_current_user)):
    chat = chat_services.get_user_chat(db, chat_id, user.id)
    if chat is None:
...
def delete_chat(chat_id: str, db: Session = Depends(get_db), user=Depends(get_current_user)):
    deleted = chat_services.delete_chat(db, chat_id, user.id)
    if not deleted:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Chat not found")
...
def send_message(chat_id: str, payload: SendMessageRequest, db: Session = Depends(get_db), user=Depends(get_current_user)):
    message = chat_services.send_user_message(db, chat_id, user.id, payload.content)
    if message is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Chat not found")
    return message
